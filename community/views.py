...
class CommunityView(APIView, PaginationHandlerMixin):
    ...
    def get(self, request):
        ...
        if id is None:
            ...
            page = self.paginate_queryset(
                article
            )  # page_size, page에 따른 pagination 처리된 결과값
        ...
        if int(id) == 3:
            ...
            if user.is_anonymous:
                return Response(
                    {"message": "로그인이 필요한 페이지입니다."}, status=status.HTTP_401_UNAUTHORIZED
                )
    ...
    def post(self, request):
        ...
        return Response(
            {"message": "게시글에 빈칸이 있습니다."}, status=status.HTTP_400_BAD_REQUEST
        )
...
class CommunityDetailView(APIView):
    ...
    def put(self, request, article_id):
        ...
        return Response(
            {"message": "게시글 작성자가 아닙니다"}, status=status.HTTP_400_BAD_REQUEST
        )
...
class CommentView(APIView):
    ...
    def post(self, request, article_id):
        ...
        if article_serializer.is_valid():
            article_serializer.save()
            return Response(
                {"message": "댓글작성 완료!", "data": article_serializer.data},
    ...
    def put(self, request, comment_id):
        ...
        comment_serializer = ArticleCommentSerializer(
