...
def calculate_distance(park_name):
    ...
    if len(park_data) >= 1:
        park_data["distance"] = park_data.apply(
            lambda x: distance.distance(
                x["park_coord"].strip("()"), x["parking_lot_coord"].strip("()")
            ).km,
            axis=1,
        )
...
class ParkCommentView(APIView):
    # 댓글 조회
    def get(self, request, park_id):
        park_comment_page = int(
            self.request.query_params.get("urlParkCommentPageNum", 1)
        )
        comment_list = ParkCommentModel.objects.filter(park_id=park_id).order_by(
            "-created_at"
        )[10 * (park_comment_page - 1) : 10 + 10 * (park_comment_page - 1)]

        comment_total_count = (
            ParkCommentModel.objects.filter(park_id=park_id)
            .order_by("-created_at")
            .count()
        )
    ...
    def post(self, request, park_id):
        if request.user.is_anonymous:
            return Response(
                {"message": "로그인을 해주세요"}, status=status.HTTP_401_UNAUTHORIZED
            )

        park = ParkModel.objects.get(id=park_id)

        data = {"park": park.id, "comment": request.data["comment"]}
    ...
    def put(self, request, comment_id):
        if request.user.is_anonymous:
            ...
            try:
                ...
                return Response(
                    {"message": "해당 댓글이 존재하지 않습니다"}, status=status.HTTP_404_NOT_FOUND
                )

            if comment.comment == request.data["comment"]:
                return Response(
                    {"message": "수정할 내용을 입력해주세요"}, status=status.HTTP_400_BAD_REQUEST
                )

            comment_serializer = ParkCommentSerializer(
            ...
            return Response(
                {"message": "내용을 입력해주세요"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def delete(self, request, comment_id):
        if request.user.is_anonymous:
            ...
            try:
                ...
                comment.delete()
                return Response(
                    {"message": "해당 댓글이 삭제되었습니다"}, status=status.HTTP_200_OK
                )
    ...
    def get(self, request):
        ...
        try:
            if len(option_list) > 0 and len(zone_list) > 0:
                results = (
                    ParkModel.objects.filter(zone__in=zone_list)
                    .filter(parkoption__option_id__in=option_list)
                    .distinct()
                )
            elif len(option_list) > 0:
                results = ParkModel.objects.filter(
                    parkoption__option_id__in=option_list
                ).distinct()
            ...
            if not results.exists():
                return Response(
                    {"message": "공원을 찾을 수 없습니다."}, status=status.HTTP_404_NOT_FOUND
                )
            elif results.exists():
...
class ParkPopularityView(APIView):
    def get(self, request):
        popular_park = ParkModel.objects.filter(check_count__gte=1).order_by(
            "-check_count"
        )
...
class ToggleParkView(APIView):
    def post(self, request):
        toggle_parks = ParkModel.objects.filter(
            addr_dong=request.data["data"]
        ).order_by("park_name")
