...
class ParkCommentTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.park = ParkModel.objects.create(
            park_name="park_name",
            addr="addr",
            image="image",
    ...
    def setUp(self):
        self.access_token = self.client.post(
            reverse("ipark_token"), self.user_data
        ).data["access"]
    ...
    def test_fail_no_comment(self):
        response = self.client.post(
            path=reverse("park_comment_create", kwargs={"park_id": 1}),
            data={"username": "sugisugi", "park": 1, "comment": ""},
            HTTP_AUTHORIZATION=f"Bearer {self.access_token}",
        )
...
class OptionTest(APITestCase):
    def create_park(self, park_data, option_list):
        ...
        for option in option_list:
            ParkOptionModel.objects.create(
                park=self.park, option=OptionModel.objects.get(id=option)
            )
