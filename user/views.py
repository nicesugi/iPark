...
class FindUserInfoView(APIView):
    ...
    def post(self, request):
        try:
            searched_username = UserModel.objects.get(
                Q(email=request.data["email"]) & Q(phone=request.data["phone"])
            ).username
            if searched_username:
                return Response(
                    {"username": searched_username}, status=status.HTTP_200_OK
                )

        except UserModel.DoesNotExist:
            return Response(
                {"message": "사용자가 존재하지 않습니다"}, status=status.HTTP_404_NOT_FOUND
            )
...
class AlterPasswordView(APIView):
    ...
    def post(self, request):
        ...
        if request.data["username"] == "" or request.data["email"] == "":
            return Response(
                {"message": "아이디 또는 이메일 값을 제대로 입력해주세요."},
            ...
            if email_input == None:
                ...
                try:
                    ...
                    return Response(
                        {"message": "존재하지 않는 사용자입니다."}, status=status.HTTP_404_NOT_FOUND
                    )
    ...
    def put(self, request):
        ...
        if request.data["new_password"] == "" or request.data["rewrite_password"] == "":
            return Response(
                {"message": "비밀번호를 제대로 입력해주세요."}, status=status.HTTP_400_BAD_REQUEST
            )
        else:
            if request.data["new_password"] == request.data["rewrite_password"]:
                ...
                if password_input == None:
                    ...
                    if check_password(request.data["new_password"], user.password):
                        return Response(
                            {"message": "현재 사용중인 비밀번호와 동일한 비밀번호는 입력할 수 없습니다."},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                    else:
            ...
            return Response(
                {"message": "두 비밀번호가 일치하지 않습니다."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class UserVerifyView(APIView):
    ...
    def post(self, request):
        ...
        if request.data["password"] == "":
            return Response(
                {"message": "비밀번호 값을 제대로 입력해주세요."}, status=status.HTTP_400_BAD_REQUEST
            )
        # 프론트에 사용자의 username이 띄워져 있지만, 사용자가 변경을 할 경우를 대비
        elif request.data["username"] == "":
            return Response(
                {"message": "아이디를 제대로 입력해주세요."}, status=status.HTTP_400_BAD_REQUEST
            )
        else:
            if password_input == None:
                return Response(
                    {"message": "비밀번호 형식에 맞게 작성해주세요."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            else:
                user = authenticate(
    ...
    def post(self, request):
        ...
        try:
            ...
            if user and (user.password == None):
                return Response(
                    {"res_code": 1, "message": "iPark 서비스 이용을 위해 회원님의 정보가 필요합니다"},
